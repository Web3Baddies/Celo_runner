...
export function ContractManager() {
  ...
  // Use Wagmi account in Farcaster mode, Thirdweb account otherwise
  const account = isMiniApp ? { address: wagmiAccount.address } : thirdwebAccount;
  const address = isMiniApp ? wagmiAccount.address : thirdwebAccount?.address;
  const isConnected = isMiniApp ? wagmiAccount.isConnected : !!thirdwebAccount;
  ...
  // Monitor thirdweb transaction states and update game store
  useEffect(() => {
  ...
  useEffect(() => {
    const callbacks = {
      registerPlayer: async (username: string): Promise<boolean> => {
        try {
          // First, check if player is already registered by reading their data
          console.log('🔍 Checking player registration status before attempting registration...');
          await hooksRef.current.refetchPlayer();
          await new Promise(resolve => setTimeout(resolve, 500)); // Wait for refetch
          
          const currentPlayer = hooksRef.current.contractPlayer;
          const hasUsername = currentPlayer?.username && String(currentPlayer.username).trim().length > 0;
          const isAlreadyRegistered = currentPlayer?.isRegistered || hasUsername;
          
          if (isAlreadyRegistered) {
            console.log('✅ Player is already registered, skipping registration attempt');
            console.log('📋 Current player data:', {
              username: currentPlayer?.username,
              isRegistered: currentPlayer?.isRegistered,
              hasUsername
            });
            // Just reload player data
            setTimeout(() => hooksRef.current.refetchPlayer(), 500);
            return true;
          }
          
          // Player is not registered, proceed with registration
          console.log('📝 Player not registered, proceeding with registration...');
          const result = await hooksRef.current.registerPlayer(username);
          
          // Check if result is the special "already_registered" value
          if (result === 'already_registered') {
            console.log('✅ Player already registered (detected during transaction), loading data...');
            setTimeout(() => hooksRef.current.refetchPlayer(), 1000);
            return true;
          }
          ...
          // Check if error is "Already registered" - this is actually fine, just reload player data
          const errorMessage = error?.message || error?.toString() || '';
          if (errorMessage.includes('Already registered') || 
              errorMessage.includes('already registered') ||
              errorMessage.includes('Player already registered')) {
            console.log('✅ Player already registered (from error), loading player data...');
            // Player is already registered, just reload their data
            setTimeout(() => hooksRef.current.refetchPlayer(), 1000);
            return true; // Return true since player is registered
          }
      ...
      loadPlayerData: async (walletAddress: string): Promise<Player | null> => {
        try {
          ...
          console.log('🔧 ContractManager - isRegistered check:', {
            ...
            hasUsername: !!(contractPlayer?.username && contractPlayer.username.trim().length > 0)
          });

          // Contract always returns player data, check isRegistered OR username to determine if they're registered
          // A player is registered if isRegistered is true OR if they have a non-empty username
          if (contractPlayer) {
            const hasUsername = contractPlayer.username && contractPlayer.username.trim().length > 0;
            const isActuallyRegistered = contractPlayer.isRegistered || hasUsername;
            ...
            if (isActuallyRegistered) {
              ...
              return {
                ...
                username: (contractPlayer.username || '').trim() || undefined,
                currentStage: Math.max(contractPlayer.currentStage || 1, 1), // Ensure at least stage 1
                totalScore: contractPlayer.totalScore || 0,
                inGameCoins: contractPlayer.inGameCoins || 0,
                tokensEarned: contractPlayer.questTokensEarned || 0,
                nftsEarned: 0, // Not tracked in contract yet
                completedStages,
                tokensClaimedStages,
                nftClaimedStages,
                claimedStages,
                totalGamesPlayed: contractPlayer.totalGamesPlayed || 0,
                isRegistered: true,
                registrationTime: contractPlayer.registrationTime || 0,
              };
            } else {
              console.log('❌ ContractManager - Player is NOT registered (no username and isRegistered=false), returning unregistered player data');
      ...
      claimTokens: async (stage: number): Promise<boolean> => {
        try {
          ...
          // Wait for transaction to be mined using thirdweb
